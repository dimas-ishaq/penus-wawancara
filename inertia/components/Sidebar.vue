...
<template>
  <div>
    ...
    <!-- Sidebar -->
    <aside 
      class="fixed inset-y-0 left-0 w-80 bg-surface-container-low border-r border-outline-variant/30 flex flex-col z-[70] transition-transform duration-500 ease-in-out lg:sticky lg:translate-x-0 lg:h-screen"
    ...
    >
      <div class="p-8 h-full flex flex-col">
        <Link href="/admin/dashboard" class="flex items-center gap-3 group mb-12 shrink-0">
          ...
          <div class="flex flex-col">
            ...
            <span class="text-on-surface-variant font-body font-bold text-[9px] tracking-[0.2em] uppercase leading-none mt-1 text-secondary">SMK PLUS PN</span>
          </div>
        </Link>

        <nav class="space-y-6">
          <div v-if="navLinks.length > 0">
            <p class="text-[10px] font-bold text-outline uppercase tracking-widest mb-4 ml-2">Main Navigation</p>
            <div class="space-y-2">
              <Link
                ...
                :class="page.url.startsWith(link.href) ? 'bg-primary text-on-primary shadow-lg shadow-primary/20' : 'text-on-surface-variant hover:bg-surface-container-high hover:text-primary'"
              >
                <span class="material-symbols-outlined transition-transform group-hover:scale-110">{{ link.icon }}</span>
                <span class="font-body font-bold tracking-tight">{{ link.name }}</span>
              </Link>
            </div>
          </div>

          <div v-if="systemLinks.length > 0">
            <p class="text-[10px] font-bold text-outline uppercase tracking-widest mb-4 ml-2">System & Branding</p>
            <div class="space-y-2">
              <Link
      ...
      <div class="mt-auto p-8 border-t border-outline-variant/30 bg-surface-container-lowest/50">
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 rounded-full bg-secondary-container flex items-center justify-center text-on-secondary-container font-black uppercase">
          ...
          <Link href="/logout" method="post" as="button" class="text-outline hover:text-error transition-colors shrink-0">
