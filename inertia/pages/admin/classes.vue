...
<template>
  ...
  <div class="space-y-8">
    <header class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
      <div>
        <h1 class="text-2xl sm:text-3xl font-black text-primary tracking-tighter font-headline mb-1 sm:mb-2 uppercase">Manajemen Kelas</h1>
        <p class="text-on-surface-variant font-body text-xs sm:text-sm">Kelola daftar kelas dan relasi jurusan di SMK Plus Pelita Nusantara.</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <template v-if="!props.classes || props.classes.length === 0">
      ...
      <template v-else>
        <div v-for="cls in filteredClasses" :key="cls.id"
          ...
          <div class="absolute top-0 right-0 p-6 opacity-[0.03] group-hover:opacity-[0.07] transition-opacity">
            <span class="material-symbols-outlined text-7xl sm:text-8xl">meeting_room</span>
          </div>
          ...
          <div class="relative z-10 space-y-4 sm:space-y-6">
            ...
            <h3 class="text-lg sm:text-xl font-black text-primary font-headline leading-tight tracking-tight">
              {{ cls.majorName }}
            </h3>
